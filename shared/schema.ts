...
// NOWPayments crypto charge tracking
export const cryptoCharges = pgTable("crypto_charges", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  paymentId: varchar("payment_id", { length: 255 }).unique().notNull(), // NOWPayments payment ID
  payAddress: text("pay_address"), // Deposit address generated by NOWPayments
  payCurrency: varchar("pay_currency", { length: 10 }).notNull(), // BTC, ETH, USDC, etc.
  payAmount: varchar("pay_amount", { length: 78 }).notNull(), // Crypto amount as string
  priceAmount: numeric("price_amount", { precision: 15, scale: 2 }).notNull(), // USD amount requested
  priceCurrency: varchar("price_currency", { length: 10 }).default("usd").notNull(),
  actuallyPaid: varchar("actually_paid", { length: 78 }), // Actual crypto amount received
  paymentStatus: varchar("payment_status", { length: 50 }).default("waiting").notNull(), // waiting, confirming, confirmed, sending, partially_paid, finished, failed, refunded, expired
  outcomeAmount: numeric("outcome_amount", { precision: 15, scale: 2 }), // Final USD credited
  networkFee: varchar("network_fee", { length: 78 }), // Network fee paid
  ipnCallbackUrl: text("ipn_callback_url"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  expiresAt: timestamp("expires_at"), // Payment expiration
  confirmedAt: timestamp("confirmed_at"), // When payment confirmed
});

// Withdrawal requests for crypto/card payouts
export const withdrawalRequests = pgTable("withdrawal_requests", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  grossAmount: numeric("gross_amount", { precision: 15, scale: 2 }).notNull(), // Amount user requested
  siteFee: numeric("site_fee", { precision: 15, scale: 2 }).notNull(), // 25% site fee
  transactionFee: numeric("transaction_fee", { precision: 15, scale: 2 }).notNull(), // 3% transaction fee
  netAmount: numeric("net_amount", { precision: 15, scale: 2 }).notNull(), // Final payout after fees
  withdrawalMethod: varchar("withdrawal_method", { length: 20 }).notNull(), // 'crypto' or 'card'
  destinationAddress: varchar("destination_address", { length: 255 }), // Crypto wallet address
  destinationCurrency: varchar("destination_currency", { length: 10 }), // BTC, ETH, USDC, etc.
  status: varchar("status", { length: 50 }).default("pending_admin_approval").notNull(), // pending_admin_approval, approved, processing, completed, rejected, failed
  transactionHash: varchar("transaction_hash", { length: 255 }), // Blockchain tx hash or payment processor ID
  adminNote: text("admin_note"), // Admin notes for review
...
// Stripe payment intents tracking
export const stripePayments = pgTable("stripe_payments", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  paymentIntentId: varchar("payment_intent_id", { length: 255 }).unique().notNull(),
  amount: numeric("amount", { precision: 15, scale: 2 }).notNull(), // USD amount
  currency: varchar("currency", { length: 10 }).default("usd").notNull(),
  status: varchar("status", { length: 50 }).notNull(), // requires_payment_method, requires_confirmation, requires_action, processing, requires_capture, canceled, succeeded
...
export const insertCryptoChargeSchema = createInsertSchema(cryptoCharges).pick({
  userId: true,
  paymentId: true,
  payAddress: true,
  payCurrency: true,
...
export const insertStripePaymentSchema = createInsertSchema(stripePayments).pick({
  userId: true,
...
export type CryptoCharge = typeof cryptoCharges.$inferSelect;
export type InsertCryptoCharge = z.infer<typeof insertCryptoChargeSchema>;
