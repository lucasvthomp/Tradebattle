...
- **Display Name System** (January 2025): Added mutable display name field to user profiles with editing interface, users can now set custom display names that appear in public profiles and people browsing instead of first/last name combination
- **Trading Performance Removal** (January 2025): Removed all trading performance metrics (trade counts, trading statistics) from public profiles and people browsing page to protect user privacy, only achievements are now visible publicly
