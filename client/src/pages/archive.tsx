...
import { Archive, Search, Trophy, Users, Crown, ArrowUpRight } from "lucide-react";
    ...
    <div className="arena-page-shell archive-page">
      <div className="mx-auto w-full max-w-5xl px-4 py-8 md:px-6 md:py-12">
        <header className="mb-7 flex items-end justify-between gap-4 border-b pb-7" style={{ borderColor: "var(--site-edge)" }}>
          <div>
            <p className="mb-2 text-[10px] font-black uppercase tracking-[0.18em]" style={{ color: "#67e7bf" }}>ARCHIVE</p>
            <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ background: "rgba(103,231,191,.1)", color: "#67e7bf" }}><Archive size={20} /></div><div><h1 className="text-2xl font-black tracking-tight md:text-3xl" style={{ color: "#eef6fa" }}>{t("archive")}</h1><p className="mt-1 text-sm" style={{ color: "#8da2b5" }}>Finished arenas and final rankings.</p></div></div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="rounded-lg border px-5 py-16 text-center" style={{ background: "#0b1b2a", borderColor: "var(--site-edge)" }}><Trophy className="mx-auto mb-3" size={30} style={{ color: "#7890a4" }} /><p className="text-sm" style={{ color: "#8da2b5" }}>{searchQuery ? t("noMatchingTournaments") : t("noCompletedTournaments")}</p></div>
        ) : (
          <div className="space-y-2">
            {filtered.map((tournament) => {
              ...
              return <a key={tournament.id} href={`/archive/${tournament.id}`} className="group flex items-center justify-between gap-4 rounded-lg border p-4 transition-colors hover:border-[rgba(103,231,191,.38)]" style={{ background: "#0b1b2a", borderColor: "var(--site-edge)" }}>
                <div className="min-w-0 flex-1"><div className="mb-2 flex flex-wrap items-center gap-2"><h2 className="truncate text-sm font-bold" style={{ color: "#eef6fa" }}>{tournament.name}</h2><Badge variant="outline" className="text-[10px]" style={{ borderColor: "rgba(103,231,191,.25)", color: "#a8ead5" }}>Completed</Badge></div><div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs" style={{ color: "#7890a4" }}><span className="inline-flex items-center gap-1"><Users size={13} />{tournament.currentPlayers}/{tournament.maxPlayers}</span><span>{tournament.endedAt ? new Date(tournament.endedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "Date unavailable"}</span>{winner && <span className="inline-flex items-center gap-1" style={{ color: "#f2c76a" }}><Crown size={13} />{winner.name}</span>}</div></div><ArrowUpRight size={17} className="shrink-0 opacity-50 transition-opacity group-hover:opacity-100" style={{ color: "#67e7bf" }} />
              </a>;
            })}
          </div>
        )}
