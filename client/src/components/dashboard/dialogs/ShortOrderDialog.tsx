...
interface ShortOrderDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export function ShortOrderDialog({
  isOpen,
  onClose,
  onConfirm,
  initialOrder,
  balance
}: ShortOrderDialogProps) {
  ...
  // Risk management - INVERTED for short positions
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="order" className="w-full">
          ...
          <TabsContent value="order" className="space-y-4 mt-4">
            ...
            <div>
              ...
              {sharesNum > 0 && (
                <p className="text-sm text-muted-foreground mt-1">
                  Margin required: {formatCurrency(totalCost)}
                </p>
              )}
            </div>
            ...
            {!canAfford && sharesNum > 0 && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  Insufficient balance. You have {formatCurrency(balance)} but need {formatCurrency(totalCost)} in margin
                </AlertDescription>
              </Alert>
            )}
          </TabsContent>
          ...
          <TabsContent value="risk" className="space-y-4 mt-4">
            {/* Stop Loss - INVERTED: triggers when price goes UP */}
            <div className="p-4 rounded-lg border border-border space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <Label className="text-base">Stop Loss</Label>
                  <p className="text-xs text-muted-foreground">
                    Automatically cover if price RISES above this level to limit losses
                  </p>
                </div>
                <Switch
            ...
            {/* Take Profit - INVERTED: triggers when price goes DOWN */}
            <div className="p-4 rounded-lg border border-border space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <Label className="text-base">Take Profit</Label>
                  <p className="text-xs text-muted-foreground">
                    Automatically cover when price FALLS below this level to lock in gains
                  </p>
                </div>
                <Switch
